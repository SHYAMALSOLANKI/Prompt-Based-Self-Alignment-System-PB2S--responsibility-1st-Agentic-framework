...
<<<<<<< HEAD
import numpy as np
from pattern_chat_dashboard import text_to_yantra, SimpleBrain, PATTERN_DIR

def extract_concepts_from_markdown(md_text):
    """
    Extracts concept blocks from markdown. Returns a list of dicts with 'title', 'content', and optionally 'equation' or 'analogy'.
    """
    concepts = []
    current = None
    for line in md_text.splitlines():
        if line.startswith('##') or line.startswith('#'):
            ...
            current = {'title': line.strip('# ').strip(), 'content': ''}
        elif line.strip().startswith('>'):
            if current:
                current.setdefault('analogy', []).append(line.strip('> ').strip())
        elif '$$' in line or '\\(' in line or '\\[' in line:
            if current:
                current.setdefault('equation', []).append(line.strip())
...
>>>>>>> 17c52f8d56fa6c10a0acbe313aa21aff4d1a67f1
    for md_file in md_files:
        try:
            ...
            continue
<<<<<<< HEAD
        concepts = extract_concepts_from_markdown(md_text)
        for concept in concepts:
            title = concept.get('title', '').strip()
            content = concept.get('content', '').strip()
            meaning = content
            if 'analogy' in concept:
                meaning += '\nAnalogy: ' + ' | '.join(concept['analogy'])
            if 'equation' in concept:
                meaning += '\nEquation: ' + ' | '.join(concept['equation'])
            if title:
                pattern = text_to_yantra(title)
                sig = brain.pattern_hash(pattern)
                brain.pattern_meanings[sig] = meaning
                brain.store_pattern(pattern, {'type': 'concept', 'value': title, 'source': md_file})
                print(f"Imported: {title}")
    # Optionally, save meanings to disk
    with open(os.path.join(PATTERN_DIR, 'imported_meanings.json'), 'w', encoding='utf-8') as f:
        json.dump(brain.pattern_meanings, f, ensure_ascii=False, indent=2)
=======
