...
def test_pb2s_structure():
    ...
    try:
        health_check = requests.get(f"{main_brain_url}/status", timeout=5)
        if health_check.ok:
            print("✅ Main Brain server is online")
        else:
            print("⚠️  Status endpoint returned non-200; attempting anyway")
    except Exception as e:
        print("❌ Main Brain server is offline")
        print("💡 Start one of: \n  - python -m uvicorn ultra_simple_chat:app --port 8000\n  - python pb2s_enhanced_server.py")
        return False
