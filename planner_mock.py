...
def chat(_req: ChatRequest):
    ...
    text=("DRAFT\n- initial take based on context\n\n"
          ...
          "LEARNED\n- concise takeaway")
    proof={"decision":"APPROVE","cycles":1,"audit_ref":f"run-{int(time.time())}-{uuid.uuid4().hex[:8]}",
           "model_sha":MODEL_SHA,"policy_sha":POLICY_SHA,"attention_state":"FAIR",
           "cr_event":{"old":"idle","new":"answer_prepared"},"bus_msg_id":str(uuid.uuid4()),
